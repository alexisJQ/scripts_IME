...
# Notemos que la función 'data.frame()' convierte variables de tipo 'string'
# en factores, que es otro nombre para variables categóricas (a menos que le
...
# codifican los posibles valores que puede tomar la variable, también llamados
# niveles ('levels' en inglés).
# Cada nivel es acompañado de un nombre, tambien llamada etiqueta ('label' en
# inglés) que es un string.
# El problema es que cuando R convierte un vector de strings a factores, asigna
# los valores en orden alfabético. Por ejemplo:
...
# no es correcto para nuestra hipótesis ?? > 25.
# Así, o cambiamos la hipótesis a ?? < 25 o cambiamos el orden de los niveles
# en el factor.
# Hagámos esto último:
...
# H1: ??_habas y ??_linaza sí son diferentes (??_habas <> ??_linaza)
...
# original en que no considera una varianza común (pooled variance),
...
cat("-----------------------------------------------------------\n")
cat("Diferencia de ??'s hipotética:", ??0, "g\n")
