...
# aunque podemos ser un poco más exigente con el ?? que usaremos (más
# pequeño).
...
# 
# Esta pregunta es sobre el parámetro ?? de la población desconocida.
...
z <- (media.muestra - ??) / (desvest.muestra / sqrt(tamaño.muestra))
p.valor <- pnorm(z)
...
# Así, los autores del artículo no podrían estar equivocados y los datos
...
t <- (media.muestra - ??) / (desvest.muestra / sqrt(tamaño.muestra))
...
# 
# Suponiendo una población de porcentajes, pregunta es su parámetro ?? 
# hipotético. Las hipótesis serían:
# H0: ?? = 3  [%]
# HA: ?? <> 3 [%]
...
# Esto ocurriría si el peso medio de las tabletas (todas las tabletas =
# población) difiere de 5 g. Luego las hipótesis serían:
# H0: ?? = 5  [g]
# HA: ?? <> 5 [g]
...
# se está cumpliendo, el tiempo promedio de activación va a sobrepasar
# los 25 s. Así:
# H0: ?? = 25 s
# HA: ?? > 25 s
...
# Si bien el el p-valor no es más pequeño que el nivel de significación
# definido, tampoco está tan lejos. La conclusión en este caso debería
# ser que la muestra debería ser incrementada para estar seguros.
